<div class="container flex flex-col items-center gap-16" id="quotation">
    ...
    <div class="flex flex-col md:flex-row items-start gap-16">
        <form id="quotationForm" action="{{ route('quotation.store') }}" method="post" class="form">
            @csrf
            <div class="flex flex-col gap-8">
                <div class="flex flex-col md:flex-row gap-8 w-full">
                    <div class="flex gap-8 flex-wrap w-full md:w-1/2">
                        @foreach($tags as $serviceId => $items)
                            @php
                                // Obtén el nombre del servicio del primer item en el grupo
                                $serviceName = $items->first()->service->title;
                            @endphp
                            <div class="w-full">
                                <h6 class="mb-4">{{ $serviceName }}</h6> <!-- Nombre del servicio -->
                                <div class="flex flex-wrap gap-4" required>
                                    @foreach($items as $tag)
                                    <div class="checkbox-container">
                                    <input type="checkbox" name="tags[]" value="{{ $tag->title }}" id="checkbox-{{ $tag->id }}" class="peer hidden">
                                    <label for="checkbox-{{ $tag->id }}" class="block bg-slate-100 px-4 py-2 rounded-full text-xs">{{ $tag->title }}</label>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
